...
import { signUp, login, googleAuth, appleAuth, clearAuthError, selectAuthStatus, selectAuthError } from '../store/authSlice';
...
const AuthPage = () => {
  ...
  useEffect(() => {
    ...
    if (!window.AppleID || !appleBtnRef.current || !appleClientId) return;
    try {
      ...
      // Apple SDK init may fail if clientId is invalid
    }
  }, []);
  ...
  const handleAppleSignIn = async () => {
    try {
      ...
      const name = response.user
        ...
        : null;
      if (idToken) {
        dispatch(appleAuth({ idToken, name }));
      }
    } catch (e) {
